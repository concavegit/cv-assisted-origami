...
def detection(img, REF):
    ...
    for goodcnt in cnts:
        ...
        for rc in ref_conts:
            approx_ref = cv2.approxPolyDP(rc,0.02*cv2.arcLength(rc,True),True)
    ...
    delevated = OR_space
    mask_off = cv2.copyMakeBorder(mask_off, 0, img.shape[0]-mask_off.shape[0], 0, img.shape[1]-mask_off.shape[1], cv2.BORDER_CONSTANT,value=0)

    try:
        if batc is not None and bat is not None:
            H, mask = cv2.findHomography(bat, batc)
        if not np.isnan(H[0,0]):
            mask_off = cv2.warpPerspective(mask_off, H, (img.shape[1],img.shape[0]))
            delevated = cv2.warpPerspective(OR_space, H, (img.shape[1],img.shape[0]))
